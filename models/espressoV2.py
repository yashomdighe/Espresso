...
class EspressoV2(L.LightningModule):
    ...
    def compute_loss(self, output_im, target_im, output_means, target_means, batch_idx, type):
        ...
        gt = np.array(self.im_tf(target_im))
        # gt.save("gt.png")
        op = np.array(self.im_tf(output_im))
        # op.save("render.png")
        cmp = Image.fromarray(np.hstack((gt, op)))
        # if not os.path.exists(f"renders/{type}/{self.current_epoch}"):
        #     os.makedirs(f"renders/{type}/{self.current_epoch}")
